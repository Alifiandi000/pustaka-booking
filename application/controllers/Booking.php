...
>getDataWhere('temp', ['id_buku' => $id_buku])->num_rows();
        ...
        $tempuser = $this->db-
>query("select*from temp where id_user ='$userid'")->num_rows();

        //cek jika masih ada booking buku yang belum diambil
        $databooking = $this->db-
>query("select*from booking where id_user='$userid'")->num_rows();
        if ($databooking > 0) {
...
{
  //mengupdate stok dan dibooking di tabel buku pada saat proses booking diselesaikan
  $this->db->query("UPDATE buku, temp SET buku.dibooking=buku.dibooking+1, buku.stok=buku.stok-1 WHERE buku.id=temp.id_buku");
    ...
    $isibooking = [
      ...
      'tgl_booking' => date('Y-m-d H:m:s'), 'batas ambil' => date(Y-m-d, strtotime('+2 days', strtotime($tglsekarang))),
      'id_user' => $where
    ];
    ...
    redirect(base_url() . 'booking_info');
}
...
public function info()
{
  ...
  and d.id_buku=bu.id and bo.id_user='$where'")->result_array();
